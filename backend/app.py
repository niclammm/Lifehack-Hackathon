...
# # ADD THIS NEW ENDPOINT FOR FILE UPLOADS (what your frontend actually uses)
...
#         # Get number of rewards from form data (default to 3)
#         num_of_rewards = int(request.form.get('num_rewards', 3))
#         print(f"Number of rewards requested: {num_of_rewards}")
...
#         # Generate recommendations if we have interaction data
#         recommended_rewards = None
#         if interactions_df is not None and len(interactions_df) > 0:
#             print(f"Generating recommendations with {num_of_rewards} rewards per user...")
#             try:
#                 recommended_rewards = train_model(interactions_df, customers_df, products_df, num_of_rewards)
#                 print("Recommendations generated successfully!")
#                 print(f"Recommendations structure: {type(recommended_rewards)}")
#                 if isinstance(recommended_rewards, dict):
#                     print(f"Recommendations keys: {list(recommended_rewards.keys())}")
#                     if 'recommendations' in recommended_rewards:
#                         print(f"Number of users with recommendations: {len(recommended_rewards['recommendations'])}")
#             except Exception as e:
#                 print(f"Error generating recommendations: {e}")
#                 traceback.print_exc()
#                 # Don't fail the entire request if recommendations fail
#                 recommended_rewards = {"error": f"Recommendations generation failed: {str(e)}"}
#         else:
#             print("No interactions data available for recommendations")
...
# Replace your existing upload_files endpoint in app.py with this:
